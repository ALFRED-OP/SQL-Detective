...
function home_index() {
    ...
    ")->fetch();
    $featuredCases = $db->query("SELECT c.*, COUNT(DISTINCT ch.id) as challenge_count
        FROM cases c LEFT JOIN challenges ch ON ch.case_id = c.id
        WHERE c.status = 'active' GROUP BY c.id ORDER BY c.difficulty ASC, c.id ASC LIMIT 6
    ")->fetchAll();
    $sqlTopics = [
    ...
    view('home', ['stats' => $stats, 'featuredCases' => $featuredCases, 'sqlTopics' => $sqlTopics]);
}

function home_how_it_works() {
    view('how-it-works');
}